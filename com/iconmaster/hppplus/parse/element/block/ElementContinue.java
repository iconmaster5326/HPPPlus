 ...
 * @author iconmaster
 */
public class ElementContinue implements Element {
    ...
    public ElementContinue(Element value,SourceRange range) {
        this.range = range;
        this.value = value;
    }
    ...
    public boolean isMatch(ArrayList parseList, int at) {
        if (parseList.get(at) instanceof TokenWord) {
            ...
            return "CONTINUE".equalsIgnoreCase((String) e.getParsedContent());
        }
        return false;
    }
    ...
    public String toString() {
        if (value!= null) {
            return "{CONTINUE: "+value+" at "+range+"}";
        } else {
            return "{CONTINUE: at "+range+"}";
        }
    }
