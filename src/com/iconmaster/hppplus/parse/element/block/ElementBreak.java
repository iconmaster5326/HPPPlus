 ...
 * @author iconmaster
 */
public class ElementBreak implements Element {
    ...
    public ElementBreak(Element value,SourceRange range) {
        this.range = range;
        this.value = value;
    }
    ...
    public boolean isMatch(ArrayList parseList, int at) {
        if (parseList.get(at) instanceof TokenWord) {
            ...
            return "BREAK".equalsIgnoreCase((String) e.getParsedContent());
        }
        return false;
    }
    ...
    public String toString() {
        if (value!= null) {
            return "{BREAK: "+value+" at "+range+"}";
        } else {
            return "{BREAK: at "+range+"}";
        }
    }
